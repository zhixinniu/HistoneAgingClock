...
############################################
## Estimation of sample-size saturation
############################################
# This script performs a MEDIPS-inspired sample-size saturation analysis
# for histone modification–based aging clock models.
#
# Description:
# - Randomly split the training samples into two independent subsets (A and B)
# - Train models on increasing numbers of samples from each subset
# - Quantify model stability as the Pearson correlation between predictions
#   generated by models trained on subsets A and B
# - Repeat the procedure multiple times to reduce stochastic noise
# - Perform the same analysis on an artificially doubled dataset to estimate
#   the theoretical upper bound of model stability (estimated saturation)
#
# Output:
# - Observed (true) saturation curve
# - Estimated saturation curve
############################################



# Randomize sample indices of the original training dataset
...
# Artificially double the dataset by duplicating indices
id_doubled <- sample(c(id_origin, id_origin))

# Split doubled dataset into two independent subsets
id_A_doubled <- id_doubled[1:floor(length(id_doubled)/2)]
id_B_doubled <- id_doubled[-id_A_doubled]


# For increasing training sample sizes,
# compute Pearson's r between predictions
# from models trained on subsets A and B
# True saturation analysis (original dataset)
...
# Estimated saturation analysis (doubled dataset)
...
# Combine results and visualize saturation curves
# Merge observed and estimated saturation results
saturation_all.df <- rbind(saturation_est.df,saturation_ori.df)

# Plot Pearson's r as a function of training sample size
